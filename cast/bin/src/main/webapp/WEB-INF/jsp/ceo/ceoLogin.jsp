                  ...
                  <div class="text-center">
					...
					<form method="post" action="/ceoLogin" id="ceoLoginForm">
	                    <div class="form-group">
	                      <input type="text" class="form-control" id="ceoId" name="ceoId"  placeholder="사업자 아이디를 입력해주세요">
	                    </div>
	                    <div class="form-group">
	                      <input type="password" class="form-control" id="ceoPw" name="ceoPw" placeholder="비밀번호를 입력해주세요">
	                    </div>
	                    <div class="form-group">
...
<script>
	...
	$('#ceoLoginBtn').click(function(){	
		if($('#ceoId').val() == '') { // id가 공백이면
			alert('ID를 입력하세요');
			return;
		} 
		if($('#ceoPw').val() == '') { // pw가 공백이면
			alert('PW를 입력하세요');
			return;
		}
		$('#ceoLoginForm').submit(); // 로그인
	});
</script>
  <!-- Login Content -->
